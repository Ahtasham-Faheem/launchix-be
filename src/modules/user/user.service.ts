...
export class UserService {
  ...
  constructor(
    ...
    private brandAssetModel: Model<BrandAssetsDocument>,
  ...
  async softDeleteUser(userId: string) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to soft delete user ${user.clerkId}: ${err.message}`,
      );
      throw err;
    }
  }
    ...
    try {
      ...
      for (const brand of brands) {
        ...
        if (this.brandAssetModel) {
          const deletedAssets = await this.brandAssetModel.deleteMany({
            brandId: brand._id,
          });
          this.logger.log(
            `Deleted ${deletedAssets.deletedCount} assets for brand ${brand.businessName}`,
          );
        }
      ...
      this.logger.warn(
        `Hard deleted user ${user.email} (${user.clerkId}) and ${brands.length} related brands`,
      );
      ...
      this.logger.error(
        `Failed to hard delete user ${user._id}: ${err.message}`,
      );
      throw err;
    }
  }
