...
2. **Context-Aware Generation**
   - If a variable name includes:
     - **"image" / "img" / "thumbnail" / "background"** → generate a *realistic* Picsum Photos URL (e.g. \`https://picsum.photos/800/600?random=1\`). Use different random numbers for each image.
     - **"icon" / "svg"** → provide an inline SVG snippet or emoji (fitting the design and tone).
     - **"color" / "gradient" / "accent"** → return a premium, visually balanced hex color or gradient CSS string.
     - **"url" / "link" / "href"** → 
       * For **navigation/menu links** (e.g., variable names containing "nav", "menu", "header", or section names like "aboutLink", "servicesLink", "contactLink", "featuresLink", "pricingLink", "teamLink", "portfolioLink") → use **anchor links** like \`#about\`, \`#services\`, \`#contact\`, \`#features\`, \`#pricing\`, \`#team\`, \`#portfolio\`, etc.
       * For **social media links** (e.g., "facebookUrl", "twitterUrl", "instagramUrl", "linkedinUrl") → return realistic social media URLs based on the brand (e.g. \`https://facebook.com/${businessName.toLowerCase().replace(/\s+/g, '')}\`, \`https://twitter.com/${businessName.toLowerCase().replace(/\s+/g, '')}\`).
       * For **external/action links** (e.g., "ctaLink", "buttonLink") → use \`#contact\` or \`#get-started\` as appropriate.
       * Example: \`"navAboutLink": "#about"\`, \`"headerServicesLink": "#services"\`, \`"facebookUrl": "https://facebook.com/brandname"\`
     - **"background" / "pattern" / "overlay"** → provide gradient or background-image CSS (e.g. \`linear-gradient(135deg, #3b82f6, #06b6d4)\`).
...
9. **Navigation Link Detection**:
   - Automatically detect navigation/menu links by analyzing variable names.
   - Common patterns to recognize as navigation links:
     * Variables containing: "nav", "menu", "header", "link"
     * Common section names: "about", "services", "contact", "features", "pricing", "team", "portfolio", "testimonials", "faq", "blog"
   - Always convert these to anchor format: \`#section-name\`
   - Use lowercase with hyphens for multi-word sections: \`#about-us\`, \`#our-services\`, \`#contact-us\`
...
- Fill every variable with an appropriate value.
- Never leave placeholders empty or incomplete.
- For navigation links, ALWAYS use anchor format (#section) not full URLs.
