...
@Processor(REGENERATE_QUEUE_NAMES.BANNER_REGENERATE, {
...
export class BannerRegenerationProcessor extends WorkerHost {
  private readonly logger = new Logger(BannerRegenerationProcessor.name);
  ...
  async process(job: Job<BannerGenerationJobData>): Promise<JobResult> {
    ...
    try {
      ...
      const prevBannersList = Array.isArray(existingAssets?.banners) ? existingAssets.banners : [];
      const existingBanner = prevBannersList.find(banner => banner.type === variant);

      // replace only the banner of the same type (variant). If not present, append.
      const updatedBanners = [...prevBannersList];
      const idx = updatedBanners.findIndex((b: any) => b?.type === banner.type);
      if (idx >= 0) {
      ...
      const update: any = {
        $set: { brand: brandObjectId, updatedAt: new Date(), banners: updatedBanners },
      };

      // push previous banners snapshot into bannersHistory if there was any previous data
      
      update.$push = { bannersHistory: existingBanner };
